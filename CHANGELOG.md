...
- Added `UpdateDomainNginxConfiguration` action to update the Nginx configuration of a domain
- Added `Nginx\UpdatePayload` payload class for Nginx configuration updates
- Added `updateNginxConfiguration()`, `getDomainNginxConfiguration()`, and `updateDomainNginxConfiguration()` methods to the Forge service and facade
...
### Fixed
- Fixed PHPStan error in `CreateCsrPayload` by removing unnecessary `array_filter` that was checking for null values that could never be null
...
### Added
- Added `hasRelationships()` and `hasLinks()` methods to `HasApiMetadata` trait for checking if relationships and links arrays are not empty
- Added GitHub pull request workflow for automated code quality checks (Pint, PHPStan, Pest, Deptrac)
